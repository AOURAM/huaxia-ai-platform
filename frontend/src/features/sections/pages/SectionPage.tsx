...
import { useSectionPosts } from '@/features/sections/hooks/useSectionPosts';
import { PostList } from '@/shared/components/post/PostList';
import type { Post, SearchPost } from '@/types/post';

export function SectionPage() {
  const pageName = 'universities';

  const { data: posts = [], isLoading, isError } = useSectionPosts(pageName);
  ...
  const visiblePosts: Post[] = searchResults ?? posts;
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setSearchError('Search failed. The backend search service may be unavailable.');
    } finally {
    ...
    <main className="grid grid-cols-1 gap-8 xl:grid-cols-[1fr_320px]">
      <section className="space-y-6">
        <motion.div
          ...
          className="rounded-[2rem] border border-brand-outline bg-white p-8 shadow-sm"
        >
        ...
        <CreatePostCard pageName={pageName} />
        ...
        <div className="rounded-[2rem] border border-brand-outline bg-white p-5 shadow-sm">
          <div className="mb-5">
            ...
            <p className="text-sm text-brand-on-surface/55">
              Real Huaxia posts about university life, study, admission, and campus experience.
            </p>
          </div>
          ...
          {isError ? (
            <p className="rounded-2xl bg-red-50 p-5 text-sm font-semibold text-red-700">
              Could not load university posts. Check the backend server.
            </p>
          ) : null}
